...
Rule changes must be previewed first. The apply call contains only a single-use ID, rechecks the current rule state, opens a local user confirmation page, validates again, makes at most one approved mutation, then reads rules back. `applyToExistingTransactions` is always false. Mutations are never retried automatically.
